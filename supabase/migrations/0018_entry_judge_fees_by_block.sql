...
add column if not exists judge_fee numeric(10, 2);

alter table public.class_template_divisions
add column if not exists default_judge_fee numeric(10, 2);
...
check (item_type in ('entry', 'judge_fee', 'stall', 'extra', 'membership', 'fee', 'discount', 'tax', 'manual'));
  ...
  select
    ...
    left(
      'Judge fee - '
        ...
        || ' / '
        || coalesce(selected_entry.division_name, 'Division'),
      255
    ),
    1,
