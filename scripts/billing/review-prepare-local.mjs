// Explicit persistent demo stack only. Additive migration and two new demo shows.
...
const dir='.tmp/review-20260907',container='supabase_db_hsp-vet-local';
...
const backup=dir+'/before-render-v2.dump';if(!existsSync(backup))writeFileSync(backup,execFileSync('docker',['exec',container,'pg_dump','-U','postgres','-Fc','postgres'],{maxBuffer:100*1024*1024}),{flag:'wx',mode:0o600});
const file='20260907000400_billing_document_render_v2.sql',version=file.split('_')[0];
if(sql(`select count(*) from supabase_migrations.schema_migrations where version='${version}'`)==='0'){
 const migration=readFileSync('supabase/migrations/'+file,'utf8').replace(/^(begin|commit);\s*$/gmi,'');sql(`begin;${migration}\ninsert into supabase_migrations.schema_migrations(version,name) values('${version}','billing_document_render_v2');notify pgrst,'reload schema';commit;`);
}
...
for(const name of ['main','staff-race']){
 const c=state.cases[name]??={show:randomUUID()};save();
 sql(`insert into shows(id,organization_id,name,slug,start_date,end_date,default_currency) select '${c.show}',organization_id,'DÉMO revue rendu 2 — ${name}','review-render-2-${c.show}',start_date,end_date,default_currency from shows where id='${f.show}' on conflict do nothing;`);
 ...
 if(!c.context){
  ...
  const config=JSON.parse(sql(`select config from billing_contexts where id='${f.context}'`));config.name_fr='Concours fictif — revue rendu 2 · '+name;config.name_en='Fictitious show — render 2 review · '+name;
