...
const dir='.tmp/review-v4',container='supabase_db_hsp-vet-local';
...
assert((await fetch('http://127.0.0.1:54332/status')).headers.get('X-HSP-Document-Render')?.split(',').includes('4'),'Restart render 4 worker first');
const backup=dir+'/before-render-v4.dump';if(!existsSync(backup))writeFileSync(backup,execFileSync('docker',['exec',container,'pg_dump','-U','postgres','-Fc','postgres'],{maxBuffer:100*1024*1024}),{flag:'wx',mode:0o600});
const file='20260907000600_billing_entry_identity_v4.sql',version=file.split('_')[0];
if(sql(`select count(*) from supabase_migrations.schema_migrations where version='${version}'`)==='0'){
 const migration=readFileSync('supabase/migrations/'+file,'utf8').replace(/^(begin|commit);\s*$/gmi,'');sql(`begin;${migration}\ninsert into supabase_migrations.schema_migrations(version,name) values('${version}','billing_entry_identity_v4');notify pgrst,'reload schema';commit;`);
}
...
for(const name of ['main']){
 ...
 sql(`insert into shows(id,organization_id,name,slug,start_date,end_date,default_currency) select '${c.show}',organization_id,'DÉMO revue rendu 4 — ${name}','review-render-4-${c.show}',start_date,end_date,default_currency from shows where id='${f.show}' on conflict do nothing;`);
 ...
 if(!c.context){
  ...
  const config=JSON.parse(sql(`select config from billing_contexts where id='${f.context}'`));config.name_fr='Concours fictif — revue rendu 4 · '+name;config.name_en='Fictitious show — render 4 review · '+name;
