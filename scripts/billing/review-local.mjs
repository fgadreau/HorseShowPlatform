...
const dir='.tmp/review-20260907',file=dir+'/state.json',state=JSON.parse(readFileSync(file)),f=JSON.parse(readFileSync('.tmp/hsp-direct/fixture.json')),a=JSON.parse(readFileSync('.tmp/hsp-direct/access.local.json'));
...
if(stage==='fund'){
 const worker=await fetch('http://127.0.0.1:54332/status');assert.equal(worker.headers.get('X-HSP-Document-Render'),'1,2','Restart the versioned document worker before creating review documents');
 for(const [name,c]of Object.entries(state.cases)){
  ...
  for(const [i,amount]of (name==='main'?[200,298.75]:[50,81.25]).entries()){
  ...
  d=await rpc('004','billing_ui_detail',{p_folio:c.folio,p_personal:true});assert.equal(d.balance,0);assert.equal(d.payments.length,2);await rpc('003','billing_set_ready',{p_folio:c.folio,p_ready:true});
 }
 console.log('Two new demo accounts funded through real local RPC: two simulated manual payments each, one HSP charge per account. No Stripe charge created.');
 ...
 for(const[name,c]of Object.entries(state.cases)){
  ...
  for(const doc of d.documents){assert.equal(doc.snapshot.render_version,2);assert(doc.snapshot.payments.every(p=>p.receipt_number));let status;
   ...
   for(const locale of ['fr','en']){const r=await req(payer,'download',doc.id,locale);assert(r.ok);const bytes=Buffer.from(await r.arrayBuffer());assert.equal(bytes.subarray(0,4).toString(),'%PDF');const path=`${dir}/pdf/${name}-${doc.kind}-${doc.number??doc.id}-${locale}.pdf`;writeFileSync(path,bytes);files.push({name,kind:doc.kind,id:doc.id,number:doc.number,locale,path,sha256:createHash('sha256').update(bytes).digest('hex')});}
