...
const fixture=JSON.parse(readFileSync('.tmp/hsp-direct/fixture.json'));
const access=JSON.parse(readFileSync('.tmp/hsp-direct/access.local.json'));
const file='.tmp/hsp-direct/cases.json';const state=existsSync(file)?JSON.parse(readFileSync(file)):{cases:{},passed:[],complete:false};
...
async function createCase(name){
 ...
 if(!c.sale){const command={context_id:fixture.context,payer_customer_account_id:c.customer,product_id:fixture.products[4].id,quantity:1,source_id:c.source};const q=await rpc(staff,'prepare_billing_operation_quote',{p_sale:command});c.sale={...command,quote_id:q.quote_id};save();}const sale=await rpc(staff,'add_documented_billing_sale',{p_request_id:c.saleKey,p_sale:c.sale,p_presentation:{section:'reservation',reservation_id:'SIMULATED-'+name,duration:'3 nuits fictives'}});c.folio=sale.account.folio_id;save();return c;
}
...
if(stage==='prepare'){
 ...
 const c=await createCase('cancel');const first=await payment({action:'begin',folio_id:c.folio,amount:'131.25',request_id:c.beginKey});c.firstAttempt=first.attempt_id;save();
 ...
 c.nextKey??=randomUUID();save();const next=await payment({action:'begin',folio_id:c.folio,amount:'131.25',request_id:c.nextKey});assert.notEqual(next.attempt_id,c.firstAttempt);c.attempt=next.attempt_id;save();state.passed.push('Actual Stripe cancellation confirmed, same amount, distinct new attempt, no payment from canceled intent');save();console.log('Cancellation and fresh same-amount attempt verified in Stripe TEST');
}else if(stage==='staff-prepare'){
 const c=await createCase('staff-first');let d=await detail(c.folio);
 if(!c.manual){c.manual={p_request_id:randomUUID(),p_payment:{folio_id:c.folio,version:d.checkout.version,amount:131.25,method:'cash',confirmed:true,reference:'DEMONSTRATION - manual simulated',received_at:new Date().toISOString(),allocations:d.charges.map(x=>({charge_id:x.id,amount:x.total}))}};save();}
...
}else if(stage==='mixed-prepare'){
 const c=await createCase('mixed');const d=await detail(c.folio);
 if(!c.manual){const charge=d.charges.find(x=>x.supplier==='association');c.manual={p_request_id:randomUUID(),p_payment:{folio_id:c.folio,version:d.checkout.version,amount:100,method:'etransfer',confirmed:true,reference:'DEMO Interac simulated only',received_at:new Date().toISOString(),allocations:[{charge_id:charge.id,amount:100}]}};save();}
 await rpc(staff,'record_billing_payment',c.manual);assert.equal((await detail(c.folio)).balance,31.25);console.log('Fictitious Interac 100 recorded; remaining 31.25 including HSP, no provider remittance inferred');
 ...
 const c=state.cases[name];assert(c?.folio);const before=await detail(c.folio);assert.equal(before.balance,0);
 if(!c.close){await rpc(staff,'billing_set_ready',{p_folio:c.folio,p_ready:true});c.recapKey??=randomUUID();save();const recap=await rpc(payer,'prepare_own_billing_recap',{p_request_id:c.recapKey,p_folio:c.folio});c.close={p_request_id:randomUUID(),p_folio:c.folio,p_version:recap.version,p_recap_id:recap.document_id};save();}
 await rpc(payer,'finalize_own_billing_folio',c.close);await rpc(payer,'finalize_own_billing_folio',c.close);
 const after=await detail(c.folio);assert.equal(after.state,'closed');assert.equal(after.balance,0);assert.equal(after.charges.length,2);assert.equal(after.documents.filter(x=>x.kind==='invoice').length,1);
 state.passed.push(name+': reservation-only account paid and finalized; replay gives one invoice and no new fee');save();console.log('Reservation-only fictitious account finalized at zero; exact replay, one invoice, one HSP fee');
}else if(stage==='check'){
 ...
 const pid=JSON.parse(readFileSync('.tmp/hsp-direct/runtime-processes.json')).listener;assert(Number.isInteger(pid)&&pid>1);const processName=execFileSync('ps',['-p',String(pid),'-o','args='],{encoding:'utf8'});assert(/(?:hsp-listener-local)\.mjs/.test(processName));
 ...
 try{
  ...
  const connected=JSON.parse(readFileSync('.tmp/hsp-direct/connect.json')).account;const pi=await stripe(`/payment_intents/${id}/confirm`,{payment_method:'pm_card_visa'},`hsp-direct-confirm-${c.attempt}`,connected);assert.equal(pi.livemode,false);assert.equal(pi.status,'succeeded');
  ...
  const events=await stripe('/events?type=payment_intent.succeeded&limit=30',undefined,undefined,connected);const event=events.data.find(e=>e.data.object.id===id);assert(event);const raw=JSON.stringify({...event,account:connected}),time=Math.floor(Date.now()/1000),sig=createHmac('sha256',config.webhook).update(`${time}.${raw}`).digest('hex');
