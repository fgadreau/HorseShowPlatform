...
const files=JSON.parse(readFileSync('.tmp/billing-pilot/documents-result.json')).files;
const docs=JSON.parse(readFileSync('.tmp/billing-pilot/integrated.json')).latest.documents;
...
try{for(const f of files){const d=docs.find(d=>d.id===f.document);assert(d);const m=documentModel(d,f.locale);result.checks++;
 ...
 assert(text.includes('Great Holly Whiz')&&text.includes('Demo Silver Star'));assert(text.includes('Stalle simulée')&&text.includes('Casquette fictive'));assert(text.includes('Taxe fictive'));result.checks+=3;
 if(d.kind==='invoice'){assert.equal(d.snapshot.total,504);assert.equal(d.snapshot.received,504);assert.equal(d.snapshot.balance,0);assert.equal(d.snapshot.payments.length,2);assert.equal(d.snapshot.charges.length,8);assert(text.includes('DEMO-INV-000001'));result.checks+=6;}
 if(d.kind==='statement'){assert(!text.includes('DEMO-INV'));result.checks++;}
 result.files.push({...f,pages:pdf.numPages});await pdf.destroy();
 }result.complete=true;console.log(JSON.stringify({complete:true,files:result.files.length,checks:result.checks}));}finally{writeFileSync('.tmp/billing-pilot/pdf-inspection.json',JSON.stringify(result,null,2));}
