...
const dir='.tmp/review-v4',file=dir+'/state.json',state=JSON.parse(readFileSync(file)),f=JSON.parse(readFileSync('.tmp/hsp-direct/fixture.json')),a=JSON.parse(readFileSync('.tmp/hsp-direct/access.local.json'));
...
if(stage==='fund'){
 const worker=await fetch('http://127.0.0.1:54332/status');assert.equal(worker.headers.get('X-HSP-Document-Render'),'1,2,3,4','Restart the versioned document worker before creating review documents');
 for(const [name,c]of Object.entries(state.cases)){
  ...
  let d=await rpc('004','billing_ui_detail',{p_folio:c.folio,p_personal:true});assert.equal(d.charges.filter(x=>x.supplier==='hsp').length,1);assert.equal(d.presentation_version,4);
 ...
 console.log('New version 4 demo account: confirmed bib references, one HSP fee and a simulated manual payment of 200.');
 ...
 for(const[name,c]of Object.entries(state.cases)){
  ...
  for(const doc of d.documents){assert.equal(doc.snapshot.render_version,4);assert(doc.snapshot.payments.every(p=>p.receipt_number));let status;
