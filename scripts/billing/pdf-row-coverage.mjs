...
export async function assertPDFRows(bytes,model,{exact=false}={}){
 ...
 try{
  ...
  for(const [row,count]of expected){const found=pages.reduce((n,text)=>n+text.split(row).length-1,0);assert(exact?found===count:found>=count,`PDF row missing or split: ${row.slice(0,180)} (expected ${count}, found ${found})`);}
