...
const dir='.tmp/review-v5',manifest=JSON.parse(readFileSync(dir+'/documents-results.json'));mkdirSync(dir+'/pdf-images',{recursive:true});let checks=0;const files=[];
for(const file of manifest.files){const bytes=readFileSync(file.path),pdf=await getDocument({data:new Uint8Array(bytes),useSystemFonts:true}).promise;let text='';
 ...
 if(file.kind==='invoice'){assert(compact.includes(file.locale==='fr'?'Détail des paiements':'Payment details'));assert(compact.includes('DEMO-RCPT'));assert(compact.includes(file.locale==='fr'?'Virement Interac':'Interac transfer'));checks+=3;}
 ...
 if(file.name==='main'){assert(compact.includes(file.locale==='fr'?'Total Bloc DEMO':'Total Bloc DEMO'));assert(compact.includes(file.locale==='fr'?'avant taxes':'before taxes'));checks++;}
