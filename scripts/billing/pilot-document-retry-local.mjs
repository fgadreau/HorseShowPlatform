...
const access=JSON.parse(readFileSync('.tmp/billing-pilot/access.local.json')),all=JSON.parse(readFileSync('.tmp/billing-pilot/cases.json')),c=all.cases.documents;
...
const pid=JSON.parse(readFileSync('.tmp/billing-pilot/processes.json')).find(p=>p.name==='documents').pid;assert(execFileSync('ps',['-p',String(pid),'-o','args='],{encoding:'utf8'}).includes('server/billing/document-server.mjs'));
...
try{
 c.statementCommand??={p_request_id:randomUUID(),p_folio:c.folio};writeFileSync('.tmp/billing-pilot/cases.json',JSON.stringify(all,null,2),{mode:0o600});const doc=await rpc(staff,'get_billing_statement',c.statementCommand);const id=doc.document_id;
...
}finally{process.kill(pid,'SIGCONT');writeFileSync('.tmp/billing-pilot/document-retry-result.json',JSON.stringify(result,null,2));}
