...
const dir='.tmp/review-v3',file=dir+'/state.json',state=JSON.parse(readFileSync(file)),f=JSON.parse(readFileSync('.tmp/hsp-direct/fixture.json')),a=JSON.parse(readFileSync('.tmp/hsp-direct/access.local.json'));
...
if(stage==='fund'){
 const worker=await fetch('http://127.0.0.1:54332/status');assert.equal(worker.headers.get('X-HSP-Document-Render'),'1,2,3','Restart the versioned document worker before creating review documents');
 for(const [name,c]of Object.entries(state.cases)){
  const entry=(block,cl,fee='entry')=>({section:'entry',block_id:block,block_label:'Bloc '+block+' — DEMO',occurrence_id:'DEMO-SESSION-A',fee_kind:fee,...(cl?{class_id:cl,class_label:cl}:{})});
  const lines=name==='main'?[[0,0,entry('12','Open L4')],[1,0,entry('12','Open L2')],[2,0,entry('12','Open L4','judge_class')],[0,1,entry('13','Open L4')],[3,1,entry('13',null,'judge_block')],[4,null,{section:'reservation',reservation_id:'REVIEW-STALL',period:'7–9 septembre / September 2026',duration:'3 nuits / nights'}],[5,null,{section:'other'}]]:[[4,null,{section:'reservation',reservation_id:'REVIEW-RACE-STALL',duration:'3 nuits / nights'}]];
  for(let i=0;i<lines.length;i++){
   const key=name+'-sale-'+i;if(!state.commands[key]){const[p,h,presentation]=lines[i],sale={context_id:c.context,payer_customer_account_id:f.customer,product_id:f.products[p].id,quantity:1,source_id:randomUUID(),beneficiary_contact_id:f.contact,...(h===null?{}:{horse_id:f.horses[h]})};const q=await rpc('003','prepare_billing_operation_quote',{p_sale:sale});await durable(key,'add_documented_billing_sale',{p_sale:{...sale,quote_id:q.quote_id},p_presentation:presentation});}
   const result=await durable(key);c.folio=result.account.folio_id;save();
  }
  let d=await rpc('004','billing_ui_detail',{p_folio:c.folio,p_personal:true});assert.equal(d.total,name==='main'?498.75:131.25);assert.equal(d.charges.filter(x=>x.supplier==='hsp').length,1);
  ...
  d=await rpc('004','billing_ui_detail',{p_folio:c.folio,p_personal:true});assert.equal(d.balance,298.75);assert.equal(d.payments.length,1);await rpc('003','billing_set_ready',{p_folio:c.folio,p_ready:true});
 }
 console.log('New demo account: seven sales, one HSP fee and one simulated manual payment of 200; 298.75 remains due.');
 ...
 for(const[name,c]of Object.entries(state.cases)){
  const d=await rpc('004','billing_ui_detail',{p_folio:c.folio,p_personal:true});assert.equal(d.state,'closed');assert.equal(d.balance,0);assert.equal(d.documents.filter(x=>x.kind==='invoice').length,1);assert.equal(d.payments.length,3);assert.equal(d.charges.filter(x=>x.supplier==='hsp').length,1);
  for(const doc of d.documents){assert.equal(doc.snapshot.render_version,3);assert(doc.snapshot.payments.every(p=>p.receipt_number));let status;
   ...
   for(const locale of ['fr','en']){const r=await req(payer,'download',doc.id,locale);assert(r.ok);const bytes=Buffer.from(await r.arrayBuffer());assert.equal(bytes.subarray(0,4).toString(),'%PDF');const path=`${dir}/pdf/${name}-${doc.kind}-${doc.number??doc.id}-${locale}.pdf`;writeFileSync(path,bytes);const coverage=await assertPDFRows(bytes,documentModel(doc,locale));files.push({coverage,name,kind:doc.kind,id:doc.id,number:doc.number,locale,path,sha256:createHash('sha256').update(bytes).digest('hex')});}
