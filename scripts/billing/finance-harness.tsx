...
function Harness(){const [profileReady,setProfileReady]=React.useState(!new URL(location.href).searchParams.has('identity-load'));const [selectedOrg,setSelectedOrg]=React.useState('org-demo');const [url,setUrl]=React.useState(location.href);React.useEffect(()=>{const fn=()=>setUrl(location.href);addEventListener('popstate',fn);return()=>removeEventListener('popstate',fn);},[]);if(new URL(url).searchParams.has('shell')){
 ...
 const props=new Proxy({activeView:financeRoute()?.view??'billing',context:profileReady?context:null,selectedOrganizationId:selectedOrg,locale:'fr',t:translations.fr,loading:false,notice:null,onViewChange:()=>{},onChangeOrganization:()=>{}},{get:(o,k)=>k in o?(o as any)[k]:async()=>{}});
 return <>{new URL(url).searchParams.has('identity-load')&&<><button onClick={()=>setProfileReady(true)}>Load fixture profile</button><button onClick={()=>setSelectedOrg('unrelated-org')}>Change unrelated association</button></>}<Dashboard {...props as any}/></>;
