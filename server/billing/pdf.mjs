...
export async function renderDocument(d,locale,{browser:provided}={}){
 ...
 try{
  ...
  await page.evaluate(({header,identity,blocks,continued,account})=>{
   ...
   newPage(true);
   for(const b of blocks){newTable(b,false);b.rows.forEach((values,i)=>{let row=tb.insertRow();values.forEach(v=>row.insertCell().textContent=v);if(body.scrollHeight>body.clientHeight){row.remove();if(!tb.rows.length)table.remove();newPage();newTable(b,i>0);row=tb.insertRow();values.forEach(v=>row.insertCell().textContent=v);if(body.scrollHeight>body.clientHeight)throw Error('BILLING_PDF_ROW_TOO_LARGE');}});}
