...
import {renderVersions} from './pdf.mjs';
...
export function startDocumentServer(env=process.env){
 ...
 const server=http.createServer(async(req,res)=>{
  res.setHeader('Cache-Control','no-store');res.setHeader('X-HSP-Document-Render',renderVersions.join(','));res.setHeader('Content-Type','application/json');res.setHeader('X-Content-Type-Options','nosniff');
  try{
