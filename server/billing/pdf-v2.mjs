import {documentModel as legacyModel,renderDocument as legacyRender} from './pdf-v1.mjs';
...
export function documentModel(d,locale){
 if(d.snapshot.render_version==null||d.snapshot.render_version===1)return legacyModel(d,locale);
 if(d.snapshot.render_version!==2)throw Error('BILLING_PDF_RENDER_VERSION');
 ...
 for(const charges of groups.values()){
  ...
  const lines=[...classes.values()].map(c=>[c.label+' — '+c.taxes.filter(Boolean).join(' · '),money(c.entry),money(c.judge),'']);lines.push(...rows,[tr('Total du bloc avant taxes — ','Block total before taxes — ')+p.block_label,money(entry),money(judge),money(entry+judge)]);
  blocks.push({label:`${tr('Inscriptions','Entries')} · ${p.block_label} — ${charges[0].horse.name} · ${p.occurrence_id}`,columns:[tr('Bloc / classe','Block / class'),tr('Inscription','Entry fee'),tr('Frais de juges','Judge fees'),tr('Total du bloc avant taxes','Block total before taxes')],rows:lines,total:entry+judge});entryTotal+=entry+judge;
 }
 ...
 if(s.suppliers){
  ...
  blocks.push({label:s.suppliers.hsp.name+' — '+tr('Service HSP','HSP service'),columns:[tr('Fournisseur / désignation','Supplier / description'),tr('Avant taxes','Before taxes'),tr('Taxes','Taxes'),tr('Total','Total')],rows:hsp.map(c=>[description(c),money(cents(c.subtotal)),money(cents(c.tax_amount)),money(cents(c.total))])});
 ...
 const method=p=>p.method==='cash'?tr('Comptant','Cash'):p.method==='etransfer'?tr('Virement Interac','Interac transfer'):p.method==='stripe_test'?tr('Carte bancaire (test)','Card (test)'):tr('Paiement','Payment');
 const date=p=>new Intl.DateTimeFormat(fr?'fr-CA':'en-CA',{dateStyle:'medium',timeStyle:'short',timeZone:'UTC'}).format(new Date(p.received_at))+' UTC';
 if(d.kind==='invoice')blocks.push({label:tr('Détail des paiements','Payment details'),columns:[tr('Date / moyen de paiement','Date / payment method'),tr('Numéro de reçu','Receipt number'),tr('Montant','Amount')],rows:s.payments.map(p=>{if(!p.receipt_number)throw Error('BILLING_PDF_RECEIPT_NUMBER_REQUIRED');return [date(p)+' — '+method(p),p.receipt_number,money(cents(p.amount))];})});
 if(d.kind==='receipt'){
  ...
  blocks.unshift({receipt:true,label:tr('Paiement concerné et affectations','Payment received and allocations'),columns:[tr('Désignation','Description'),tr('Montant total du frais (taxes incluses)','Full charge amount (including taxes)'),tr('Portion réglée par ce paiement','Portion paid by this payment')],rows:[[date(p)+' — '+method(p)+' — '+d.number,'',money(cents(p.amount))],...p.allocations.map(a=>{const c=s.charges.find(c=>c.id===a.charge_id);if(!c)throw Error('BILLING_PDF_INVALID_RECEIPT');return [[description(c),c.presentation?.block_label,c.presentation?.occurrence_id].filter(Boolean).join(' · '),money(cents(c.total)),money(cents(a.amount))];})]});
 }
...
export async function renderDocument(d,locale,{browser:provided}={}){
 if(d.snapshot.render_version==null||d.snapshot.render_version===1)return legacyRender(d,locale,{browser:provided});
