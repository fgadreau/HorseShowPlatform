import {documentModel} from './pdf-model-v5.mjs';
...
export async function renderDocument(d,locale,{browser:provided}={}){
 if(d.snapshot.render_version!==5)throw Error('BILLING_PDF_RENDER_VERSION');
 ...
 try{
  ...
  await page.evaluate(({header,identity,blocks,continued,account,renderLabel})=>{
   ...
   function newTable(block,continuation){table=document.createElement('table');if(block.endsBib)table.className='ends-bib';const cg=document.createElement('colgroup');block.columns.forEach((_,i)=>{const c=document.createElement('col');c.style.width=(i===0?55:45/(block.columns.length-1))+'%';cg.append(c);});table.append(cg);const head=table.createTHead();let row=head.insertRow();row.className='group';let th=document.createElement('th');th.colSpan=block.columns.length;th.textContent=block.label+(continuation?' — '+continued:'');row.append(th);row=head.insertRow();block.columns.forEach(v=>{const th=document.createElement('th');th.textContent=v;row.append(th);});tb=table.createTBody();body.append(table);}
   ...
   for(const b of blocks){
    newTable(b,false);
    // The last detail, block subtotal and (on the last block) bib total form
    // one atomic group. Measure the whole group before committing its page.
    const tail=b.keepTail??1,split=b.rows.length-tail;
    for(let i=0;i<b.rows.length;){
     const end=i>=split?b.rows.length:i+1;
     const append=()=>b.rows.slice(i,end).map((values,j)=>{const row=tb.insertRow();row.dataset.block=String(blocks.indexOf(b));row.dataset.index=String(i+j);row.className=b.rowKinds?.[i+j]??((b.receipt?i+j===0:i+j===b.rows.length-1)?'total':'');values.forEach(v=>row.insertCell().textContent=v);return row;});
     let rows=append();
     if(body.scrollHeight>body.clientHeight){
      rows.forEach(r=>r.remove());if(!tb.rows.length)table.remove();
      newPage();newTable(b,i>0);rows=append();
      if(body.scrollHeight>body.clientHeight)throw Error('BILLING_PDF_KEEP_GROUP_TOO_LARGE');
     }
     i=end;
    }
   }
   ...
   for(const [bi,b] of blocks.entries()){if(!b.keepTail)continue;const tail=rows.filter(r=>Number(r.dataset.block)===bi&&Number(r.dataset.index)>=b.rows.length-b.keepTail);if(tail.length!==b.keepTail||new Set(tail.map(r=>r.closest('.page'))).size!==1)throw Error('BILLING_PDF_ISOLATED_TOTAL');}
