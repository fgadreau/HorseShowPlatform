...
export async function stripeAction(body:Record<string,unknown>){
 ...
 const res=await fetch(import.meta.env.VITE_DEPLOY_ENV==='local'?'/__local-billing/payment':'/api/billing/payment',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${data.session?.access_token??''}`},body:JSON.stringify(body)});
