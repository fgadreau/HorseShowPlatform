...
export function DocumentView({document,locale,personal=true}:{document:Json;locale:string;personal?:boolean}){
 ...
 {s.suppliers&&<div>{Object.entries(s.suppliers).map(([key,value])=>{const v=value as Json;return <p key={key}><strong>{v.name}</strong> · {v.address} · {v.tax_number_1} {v.tax_number_2}{key==='hsp'&&<small>{fr?' — Facturé et encaissé par l’association comme mandataire':' — Billed and collected by the association as agent'}</small>}</p>;})}</div>}<div className="finance-table-wrap"><table><thead><tr><th>{fr?'Frais':'Charge'}</th><th>{fr?'Bénéficiaire / cheval':'Beneficiary / horse'}</th><th>{fr?'Taxes':'Taxes'}</th><th>Total</th></tr></thead><tbody>{s.charges.map((c:Json)=><tr key={c.id}><td>{s.suppliers?.[c.supplier]?.name}<br/>{c.supplier==='hsp'&&!fr?'HSP service fee':c.description} × {c.quantity}</td><td>{c.beneficiary?.display_name??'—'} / {c.horse?.name??'—'}</td><td>{c.taxes.map((t:Json)=><div key={t.code}>{t.name} ({t.rate}%) : {money(t.amount)}</div>)}</td><td>{money(c.total)}</td></tr>)}</tbody></table></div>
