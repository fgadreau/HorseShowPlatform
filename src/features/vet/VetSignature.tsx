...
export function VetAuthorizationPanel({practitionerId,admin=false}:{practitionerId:string;admin?:boolean}) {
 ...
 async function request(method:string){if(busy)return;setBusy(true);setNotice('');try{const r=await vetWorker<{token?:string}>('authorization-request',{practitioner_id:practitionerId,method,email});setToken(r.token??'');await load();if(!r.token)setNotice(vetLocalServices?'Lien personnel capturé dans Mailpit. Aucun envoi externe.':'Lien personnel capturé dans la boîte privée de test PREPROD. Aucun envoi externe.');}catch(e){setNotice(e instanceof Error?e.message:'Demande impossible.');}finally{setBusy(false);}}
 if(!services.ready)return <div className="vet-guide"><h4>En attente de vérification</h4><p>Le service de vérification et de signature n’est pas encore disponible dans cet environnement. Vous pouvez préparer et enregistrer le brouillon.</p></div>;
 ...
 {!active&&<fieldset disabled={busy}><button onClick={()=>void request('clinic_device')}>Le vétérinaire autorise sur cet appareil</button><label>Courriel personnel du vétérinaire<input type="email" value={email} onChange={e=>setEmail(e.target.value)}/></label><button onClick={()=>void request('personal_link')}>{vetLocalServices?'Envoyer un lien d’autorisation — test local':'Capturer un lien d’autorisation — test PREPROD'}</button></fieldset>}
